...
#define ALGO 0
#define FILTER 0
#define NO_DOWNSCALE false
#define WINDOW_SIZE -1
#define LAMBDA 8000.0
#define SIGMA 1.5
#define VIS_MULT 2.0
...
enum Filter {
	wls_conf = 0, wls_no_conf = 1
};
int main(int argc, char** argv)
{
	//Step 1
	Mat left = imread("left_view.jpg", IMREAD_COLOR);
	if (left.empty())
	{
		cout << "Cannot read image file: left_view.jpg";
		return -1;
	}

	Mat right = imread("right_view.jpg", IMREAD_COLOR);
	if (right.empty())
	{
		cout << "Cannot read image file: right_view.jpg";
		return -1;
	}

	string dst_path = "dst_view.jpg";
	string dst_path2 = "disparity_image.jpg";

	Algoritmo algo;

	if (ALGO >= 2 || ALGO < 0) {
		cout << "This algorithm doesn´t exist ";
		return -1;
	}
	else if (ALGO == 0)
		algo = BM;
	...
	else
	{
		...
		else if (!NO_DOWNSCALE && ALGO == 0 && FILTER == 0)
	...
	*/
	...
	if (FILTER == 0) // filtering with confidence (significantly better quality than wls_no_conf)
	{
		if (!NO_DOWNSCALE)
		{
		...
		if (ALGO == 0)
		{
			...
			//Pasar las imagenes a blanco y negro (sino compute no se puede realizar con exito)
	...
	*/
	...
	imwrite(dst_path2, raw_disp_vis);
	//Mat filtered_disp_vis;
	getDisparityVis(filtered_disp, filtered_disp_vis, VIS_MULT);
